				...
				<h2>站内信<span style="float:right"><a href="message_send.htm">发送站内信</a>&nbsp;&nbsp;</span></h2>
					...
					<c:forEach items="${messages }" var="msg">
						<li class="fn-clear">
							...
							<c:if test="${empty msg.imagepath }">
								<a href="#"><img width="75" height="75" src="../images/default.jpg"> </a>
							</c:if>
							<c:if test="${not empty msg.imagepath }">
								<a href="#"><img width="75" height="75" src="../images/${msg.imagepath }"> </a>
							</c:if>
							</div>
							<div class="comment_info fn-right">
								<p>
									<a class="comment_user" href="#"> ${msg.name }</a>${msg.time }
									<label style="float:right"><a class="comment_user" href="delete_msg.do?id=${msg.id }&fromId=${msg.from_id}">删除</a></label>
								...
								<div class="comment_box">
									...
									<div class="comment_cnt">${msg.content } <a href='#' >#一起玩旅游圈之试玩团心声#</a> 
					...
					<div class="no-pages cf">
						<c:if test="${currentPage>1 }">
							<a href="message.do?p=${currentPage-1 }" class="pageBtn">上一页 &lt;&lt;</a>
						</c:if>
						<c:forEach begin="1" end="${length }" step="1" var="i">
							...
							<c:if test="${currentPage!=i }">
								<a href="message.do?p=${i }">${i }</a>
							</c:if>
						</c:forEach>
						<c:if test="${currentPage<length }">
							<a href="message.do?p=${currentPage+1 }" class="pageBtn">下一页 &gt;&gt;</a>
						</c:if>
					</div>
